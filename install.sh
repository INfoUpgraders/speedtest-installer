...
if [ -x "$(command -v speedtest)" ]; then
    ...
    then
        ...
        echo -e "deb https://ookla.bintray.com/debian generic main" | tee  /etc/apt/sources.list.d/speedtest.list
        apt update -y
    ...
    then
        echo -e "${CYAN} Detected Arch Linux installation."
        sleep 2
        sudo pacman -Syu --noconfirm && sudo pacman -S speedtest-cli --noconfirm
        ${BRK}
        echo -e "${CYAN} Speedtest has been installed. Use speedtest as the command to control it."
        ${BRK}
        sleep 3
        ...
        echo -e "${CYAN} Non-compatible installation detected. Yum/pkg/apt/brew/yay were not found in path."
    fi
fi
echo -e "${RESET} "
